...
class Administrativo extends Component {
  ...
  componentDidMount() {
    ServiceAxios.get("administrativo").then(response => {
        this.setState({administrativo: response.data.items});},
      error => {console.log(error.response.data.message);}
    );
  }
  redirectEditar(index){
    return this.props.history.push(`/administrativo/${index}`);
  }
    render(){
      return (
        <div>
          <Header />
            <div className="wrapper">
              {this.state.administrativo?.map((user) => (
              <div className='card'>
